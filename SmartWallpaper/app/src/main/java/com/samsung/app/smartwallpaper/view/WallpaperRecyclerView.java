    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(diffX > 400 && diffTime < 500 && Math.abs(diffY)<Math.abs(diffX) && !canScrollHorizontally(-1)){
                    if(mCb != null) {
                        mCb.onSwipe(true);
                        return true;
                    }
                }else if(diffX < -400 && diffTime < 500 && Math.abs(diffY)<Math.abs(diffX) && !canScrollHorizontally(1)){
                    if(mCb != null) {
                        mCb.onSwipe(false);
                        return true;
                    }
